...
const MarkController = compose(withFive(FEATURES))(
  class extends Component<Props, State> {
    ...
    onTapGesture: EventCallback['wantsTapGesture'] = (raycaster) => {
      if (this.state.active) {
        ...
        if (intersect)
          this.setState({ marks: this.state.marks.concat(intersect.point) });
        return false;
      }
    };
    ...
    render() {
      return (
        <Paper sx={{ position: 'fixed', top: 60, left: 10, padding: 1 }}>
          <Stack>
            <Stack direction='row'>
              <Switch
            ...
            <Stack spacing={1}>
              {this.state.marks.map((point, index) => {
                ...
                return (
                  <Chip
                    key={index}
                    label={`x=${x.toFixed(2)} y=${y.toFixed(2)} z=${z.toFixed(
                      2
                    )}`}
                    onDelete={() =>
                      this.setState({
                        marks: this.state.marks.filter(
